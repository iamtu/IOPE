...
class runStreamingOPE:
    ...
    def run(self):
        ...
        streaming_ope = Streaming_OPE.StreamingOPE(self.settings['num_terms'], self.settings['num_topics'], self.settings['alpha'], 
        ...
        while i < self.settings['iter_train']:
            ...
            while True:
                ...
                (time_e, time_m, theta) = streaming_ope.static_online(wordids, wordcts)
                # Compute sparsity
                sparsity = utilities.compute_sparsity(theta, theta.shape[0], theta.shape[1], 't')
                # Compute perplexities
                LD2 = utilities.compute_perplexities_vb(streaming_ope._lambda, self.settings['alpha'], self.settings['eta'], 
                                                        self.settings['iter_infer'], self.test_data)
                # Search top words of each topics
                list_tops = utilities.list_top(streaming_ope._lambda, self.tops)
                # Write files
                utilities.write_file(i, j, streaming_ope._lambda, time_e, time_m, theta, sparsity, LD2, list_tops, self.tops, 
                                     self.model_folder)
            datafp.close()
        # Write settings
        print'write setting ...'
